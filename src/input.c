...
/* Keys that went down since the last poll, translated where we can. */
int input_getchar(void)
{
    int i, j;

    for (i = 0; i < kbd.nkeys; i++) {
        int fresh = 1;
        ...
        for (j = 0; j < 3; j++)
            if (kbd_last[j] == kbd.keys[i])
                fresh = 0;
        if (fresh) {
            int c = input_key_mapped(kbd.keys[i]);

            for (j = 0; j < 3; j++)
                kbd_last[j] = (j < kbd.nkeys) ? kbd.keys[j] : 0;
            if (c)
                return c;
            return 0;
        }
